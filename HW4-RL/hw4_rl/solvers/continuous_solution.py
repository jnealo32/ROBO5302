...
class DiscretizedSolver:
    ...
    def get_discrete_state_probabilities(self, continuous_state: np.ndarray) -> List[Tuple[int, float]]:
        ...
        if self._mode == "nn":
            ...
            # - this functionality already exists in the code, so I am just using the included helper functions
            return [(self.get_state_index_from_coordinates(continuous_state), 1.0)]  # This function

        else:  # linear interpolation
            # Get neighboring grid points
            bin_sizes = self.bin_sizes  # Size of each bin
            state_lower_bound = self.state_lower_bound  # Lower bound of continous space
            state_upper_bound = self.state_upper_bound  # Upper bound of continous space

            offsets = np.array( # Generates 3x3 matrix of neigher points 
            ...
            for coord in neighbor_coords:
                if np.all((coord >= state_lower_bound) & (coord <= state_upper_bound)):    # If coord is in bounds  
                    valid_neighbors.append(coord)   # If neighbor coord is valid, add coord to valid_neighbors

            # Convert to state indices and calculate interpolation weights
            interpolation_weights = []
            state_indices = []
            for neighbor in valid_neighbors:
                state_indices.append(self.get_state_index_from_coordinates((neighbor)))         
                weight = np.prod(1 - np.abs((neighbor - continuous_state) / bin_sizes))    # np.prod ensures final weight is the combined influence across all states, 1- ensures the closer the state is the higher it's weight
                interpolation_weights.append(weight)
            
            total_weight = sum(interpolation_weights)
            if total_weight > 0:    # If greater than 1
                normalized_weights = []
                for i in interpolation_weights:
                    normalized_weights.append(i / total_weight) # Normalize weights so total_weight = 1
                weights = normalized_weights

            return list(zip(state_indices, weights))  # Return tuple of indiex and associate weight
    ...
    def _value_iteration(self, max_iterations: int, min_iter: int, eval_sample_size: int) -> None:
        ...
        num_s = self.solver.num_states
        num_a = self.solver.num_actions

        R = self.solver._reward_function
        P = self.solver._transition_function
        ...
        for i in range(max_iterations):
            ...
            for s in range(self.solver.num_states):
                ...
                for a in range(self.solver.num_actions):
                    new_value = np.sum(P[s,a,:]*(R[s,a,:] + self.gamma*v_old))
                    if new_value > max_value:
                        max_value = new_value
                value_function[s] = max_value

            # Update value function and policy
            Q = np.sum(P * (R + self.gamma * value_function[None, None, :]), axis=2)
            best_actions = np.argmax(Q, axis=1)
            policy[:] = 0.0
            policy[np.arange(self.solver.num_states), best_actions] = 1.0

            # Check convergence
            value_diff = np.max(np.abs(v_old - value_function))
    ...
    def _deterministic_policy_iteration(
        ...
        for k in range(max_iterations):
            ...
            self.solver.set_policy_function(policy)
            self.solver.set_value_function(value_function)
            reward, steps = self.solve(max_steps=200, sample_size=eval_sample_size, visualize=True)
            self.performance_history.append(reward)
...
if __name__ == "__main__":
    ...
    for n_bins in bin_sizes:
        ...
        for policy_type, label, temp in algorithms:
            ...
            solver.compute_policy(max_iterations=200)
            elapsed_time = time.time() - start_time
            print(f"Computed {label} Policy in {elapsed_time:.2f} seconds")
            solver.solve(max_steps=200, sample_size=1, visualize=True)
            # Plot results
            solver.plot_value_function(
                solver.solver.get_value_function(),
                os.path.join(
                    figures_dir, f"mountaincar_{policy_type}_value_{solver._mode}_{n_bins}.png"
                ),
            )
            solver.plot_policy(
                os.path.join(
                    figures_dir, f"mountaincar_{policy_type}_policy_{solver._mode}_{n_bins}.png"
                )
            )
        ...
        plot_policy_curves(
            bin_histories,
            bin_labels,
            os.path.join(figures_dir, f"mountaincar_learning_curves_{solver._mode}_{n_bins}_bins.png"),
        )
