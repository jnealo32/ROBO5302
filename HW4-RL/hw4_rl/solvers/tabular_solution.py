...
class GridworldSolver:
    ...
    def plot_policy_curve(
        ...
        if filename is None:
            ...
            filename = os.path.join(figures_dir, f"{self.env_name}_{self._policy_type}_learning_curve.png")
    ...
    def _value_iteration(self) -> None:
        ...
        for _ in range(horizon):
            # Student code here     
            delta = 0
            v_new = np.zeros(unwrapped_env.num_states)
            for s in range(unwrapped_env.num_states):
                v = []
                for a in range(unwrapped_env.num_actions):
                    for s_next in range(unwrapped_env.num_states):
                        v.append(T[s, a, s_next]*(R[s,a,s_next] + self.gamma * v_i[s_next]))
                v_i[s] = np.max(v)
                delta = max(delta, abs(v_i[s]-v_new[s]))
            
            pass  # Placeholder for student implementation

            # Update policy
            old_best = np.argmax(p_i, axis=1) 

            # calculating state-action-value for each s, a
            Q = np.sum(T * (R + self.gamma * v_i[None, None, :]), axis=2)
            ...
            # Check convergence 
            if np.array_equal(old_best, best_actions):
                print("......Policy stable; stopping VI")
    ...
    def _deterministic_policy_iteration(self) -> None:
        ...
        for k in range(50):
            ...
            for _ in range(horizon):
                ### STUDENT CODE ###
                actions = np.argmax(p_i, axis=1) # treat policy as deterministic by picking most likely action for each state
                v_new = np.zeros(unwrapped_env.num_states)
                delta = 0
                for s in range(unwrapped_env.num_states):
                    for s_next in range(unwrapped_env.num_states):
                        v_new[s] += T[s, actions[s],s_next]*(R[s,actions[s],s_next] + self.gamma * v_i[s_next])
                    delta = max(delta, np.abs(v_i[s] - v_new[s]))
                # Check for convergence
                # delta = np.max(delta, np.abs(v_new[s] - v_i[s]))
                v_i = v_new
                if delta < self.theta:
                    break
            ...
            # Policy Improvement
            # Student code here
            old_best = np.argmax(p_i, axis=1) # used to check for convergence
            ...
            if np.array_equal(old_best, best_actions):
                print("......Policy stable; stopping PI")
                self.solver.set_policy_function(p_i)
                self.solver.set_value_function(v_i)
                return
            pass  # Placeholder for student implementation
...
if __name__ == "__main__":
    ...
    for solver in [gw0_solver, gw1_solver]:
        ...
        print("Computed Q1.1 PI Policy in %g seconds" % elapsed_time)
        solver.plot_value_function(solver.solver.get_value_function())
        solver.plot_policy(solver.compute_policy())
    ...
    for solver in [gw0_det_solver, gw1_det_solver]:
        ...
        solver.compute_policy()
        elapsed_time = time.time() - start_time
        print("Computed Q1.2 VI Policy in %g seconds" % elapsed_time)
        solver.plot_value_function(solver.solver.get_value_function())
        solver.plot_policy(solver.compute_policy())
        solver.plot_policy_curve(solver.performance_history)