...
class ParticleFilter:
    ...
    def predict(self, action: np.ndarray):
        ...
        std_dev_v = self.alpha1 * abs(forward_vel)  # since variance (alpha^2 * vel^2) is square of std dev
        std_dev_w = self.alpha2 * abs(angular_vel)

        for i in range(self.n_particles):
            x, y, theta = self.particles[i]

            noisy_vel = forward_vel + np.random.normal(0, std_dev_v)
            noisy_angular_vel = angular_vel + np.random.normal(0, std_dev_w)

            dx = noisy_vel * np.cos(theta) * dt
            dy = noisy_vel * np.sin(theta) * dt
            dtheta = noisy_angular_vel * dt

            x_new = x + dx
            y_new = y + dy
            theta_new = (theta + dtheta + np.pi) % (2 * np.pi) - np.pi  # normalize theta between [-pi, pi]

            while self.env._check_collision(np.array([x_new, y_new])):  # ensure that new particles are not within an obstacle
                x_new = np.random.uniform(0, self.env.size[0])
                y_new = np.random.uniform(0, self.env.size[1])
    ...
    def update(self, measurements: np.ndarray):
        ...
        sum_of_weights = 0.0
        for i in range(self.n_particles):
            measurements_exp = self._get_expected_measurements(self.particles[i])
            errors = measurements - measurements_exp
            likelihood = 1.0
            for j in range(self.env.n_beams):
                p = np.exp(-0.5 * (errors[j] / self.sigma_range)**2)
                likelihood *= p
            self.weights[i] *= likelihood
            sum_of_weights += self.weights[i]

        if sum_of_weights == 0:         # safety check, avoid 0 division error
            self.weights[:] = 1.0 / self.n_particles
        else:
            self.weights = self.weights / sum_of_weights
    ...
    def resample(self):
        ...
        noise = np.random.normal(0, self.resampling_noise, size=self.particles.shape)
    ...
    def estimate_state(self) -> np.ndarray:
        ...
        wmean_x = self.particles[:,0] @ self.weights.T
        wmean_y = self.particles[:,1] @ self.weights.T

        cos_vec = np.cos(self.particles[:,2])
        sin_vec = np.sin(self.particles[:,2])
        wmean_cos = cos_vec @ self.weights.T
        wmean_sin = sin_vec @ self.weights.T
        
        wmean_theta = np.atan2(wmean_sin,wmean_cos)

        state = np.array([wmean_x,wmean_y,wmean_theta])
        return state
