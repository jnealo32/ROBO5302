...
class ExtendedKalmanFilter:
    ...
    def _motion_model(self, state: np.ndarray, forward_vel: float, angular_vel: float, dt: float) -> np.ndarray:
        ...
        next_state = np.array([x_next, y_next, heading_next])
    ...
    def _motion_jacobian(self, state: np.ndarray, forward_vel: float, angular_vel: float, dt: float) -> np.ndarray:
        ...
        vel_scale = self.env._get_velocity_scaling(state[:2])
        dvs_dx = 0.5 * freq * np.cos(freq * x) * np.sin(freq * y)
        dvs_dy = 0.5 * freq * np.sin(freq * x) * np.cos(freq * y)
        effective_vel = forward_vel * vel_scale
        speed = np.abs(effective_vel) / self.env.max_velocity
        speed_factor = np.clip(speed, 0.5, 1.5)
        effective_angular_vel = angular_vel / speed_factor
        ...
        # build jacobian
        F = np.eye(3)
        F[0,0] = 1 + dvs_dx * forward_vel * np.cos(theta) * dt
        F[0,1] = dvs_dy * forward_vel * np.cos(theta) * dt
        F[0,2] = effective_vel * (-np.sin(theta)) * dt

        F[1,0] = dvs_dx * forward_vel * np.sin(theta) * dt
        F[1,1] = 1 + dvs_dy *  forward_vel * np.sin(theta) * dt
        F[1,2] = effective_vel * np.cos(theta) * dt

        F[2,0] = -dt * angular_vel * forward_vel * dvs_dx / (self.env.max_velocity * speed_factor**2)  # quotient rule
        F[2,1] = -dt * angular_vel * forward_vel * dvs_dy / (self.env.max_velocity * speed_factor**2)
        F[2,2] = 1.0
    ...
    def _measurement_jacobian(self, state: np.ndarray) -> np.ndarray:
        ...
        dx, dy = state[:2] - self.env.station[:2]
        r = np.sqrt(dx**2 + dy**2)

        if r < 1e-9:    # avoid division by 0 error
            return np.zeros((1,3))
    ...
    def update(self, measurements: np.ndarray):
        ...
        z_expected = self._measurement_model(self.state) 

        H = self._measurement_jacobian(self.state)      # measurement jacobian
        S = H @ self.covariance @ H.T + self.R          # innovation covariance
        K = self.covariance @ H.T @ np.linalg.inv(S)    # kalman gain

        y = z - z_expected                   # innovation
        dx = np.clip((K @ y).reshape(-1), -self.max_position_update, self.max_position_update)
        
        self.state = self.state + dx                    # update
